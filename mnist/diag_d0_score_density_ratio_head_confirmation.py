"""Normalized-head coordinate repair for D0 density-ratio controls.

The paired-mixture pilot localized its clipping pathology to the spatial-sum
scalar head.  This additive workflow uses an exactly function-equivalent
spatial-mean coordinate and a conjugate AdamW parameterization.  It remains
strictly controls-only: it never trains on physical states and never imports
or invokes a reverse sampler.
"""
...
RUN_SCHEMA = "experiment12-d0-score-density-ratio-head-confirmation"
RUN_SCHEMA_VERSION = 1
CHECKPOINT_SCHEMA_VERSION = 1
CLAIM_SCOPE = "normalized-head paired bounded synthetic density-ratio controls only"
...
DEFAULTS: dict[str, Any] = {
    **EXPECTED_KERNEL,
    ...
    "confirm_model_seeds": (260891, 260892, 260893),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--parent-paired-ratio-run-dir", type=Path, required=True)
    ...
    parser.add_argument(
        "--run-name", default="production-normalized-head-density-ratio-controls"
    )
    parser.add_argument(
...
def _scientific_config(
    ...
    value = {
        ...
        "pilot": {
            ...
            "profile_selection": "fixed-accumulation-8-two-learning-rate-pilot",
...
def _task_fingerprints(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "loss_scale": float(PARENT_LOSS_SCALE),
        "grid_cells": int(
            dict(manifest["scientific_config"])["kernel"]["grid_size"]
        ) ** 2,
        "body_weight_decay": float(
            dict(manifest["scientific_config"])["optimization"][
                "body_weight_decay"
            ]
        ),
        "adamw_epsilon": 1e-8,
...
def _load_checkpoint(
    ...
    *, device: torch.device, task: str, fingerprints: Mapping[str, Any]
    ...
    required = {
        ...
        "paired_stream_plan", "fingerprints", "optimizer_parameter_groups",
    }
    if not required.issubset(value):
        raise ArtifactCompatibilityError("paired checkpoint is incomplete")
    groups = [dict(group) for group in value["optimizer_parameter_groups"]]
    if len(groups) != 2 or [group.get("name") for group in groups] != [
        "body", "normalized_head"
    ]:
        raise ArtifactCompatibilityError("normalized-head optimizer groups changed")
    body_lr = float(groups[0].get("lr", math.nan))
    head_lr = float(groups[1].get("lr", math.nan))
    factor = int(round(head_lr / body_lr)) if body_lr > 0.0 else -1
    epsilon = float(groups[0].get("eps", math.nan))
    weight_decay = float(groups[0].get("weight_decay", math.nan))
    if factor <= 0:
        raise ArtifactCompatibilityError("normalized-head coordinate factor is invalid")
    if "grid_cells" in fingerprints and factor != int(fingerprints["grid_cells"]):
        raise ArtifactCompatibilityError("normalized-head coordinate factor changed")
    expected_groups = (
        (body_lr, epsilon, weight_decay),
        (factor * body_lr, epsilon / factor, weight_decay / factor),
    )
    for group, expected in zip(groups, expected_groups, strict=True):
        for key, target in zip(("lr", "eps", "weight_decay"), expected, strict=True):
            if not math.isclose(
                float(group.get(key, math.nan)), float(target), rel_tol=0.0, abs_tol=0.0
            ):
                raise ArtifactCompatibilityError(
                    f"normalized-head optimizer group {key} changed"
                )
        if tuple(float(item) for item in group.get("betas", ())) != (0.9, 0.999):
            raise ArtifactCompatibilityError("normalized-head optimizer betas changed")
        if int(group.get("amsgrad", -1)) != 0:
            raise ArtifactCompatibilityError("normalized-head optimizer AMSGrad changed")
    if int(groups[0].get("parameter_count", -1)) <= 0 or int(
        groups[1].get("parameter_count", -1)
    ) != 2:
        raise ArtifactCompatibilityError("normalized-head optimizer partition changed")

    optimizer_state = value.get("optimizer_state_dict")
    if not isinstance(optimizer_state, Mapping):
        raise ArtifactCompatibilityError("normalized-head optimizer state is invalid")
    state_groups = optimizer_state.get("param_groups")
    if not isinstance(state_groups, list) or len(state_groups) != len(groups):
        raise ArtifactCompatibilityError("normalized-head optimizer state groups changed")
    for state_group, recorded_group in zip(state_groups, groups, strict=True):
        if not isinstance(state_group, Mapping):
            raise ArtifactCompatibilityError("normalized-head optimizer state group is invalid")
        for key in ("name", "lr", "eps", "weight_decay", "betas", "amsgrad"):
            actual = state_group.get(key)
            expected = recorded_group.get(key)
            if key == "betas":
                actual = tuple(float(item) for item in (actual or ()))
                expected = tuple(float(item) for item in (expected or ()))
            elif key in {"lr", "eps", "weight_decay"}:
                actual = float(actual) if actual is not None else math.nan
                expected = float(expected) if expected is not None else math.nan
            elif key == "amsgrad":
                actual, expected = int(bool(actual)), int(bool(expected))
            if actual != expected:
                raise ArtifactCompatibilityError(
                    f"normalized-head optimizer state group {key} changed"
                )
        parameters = state_group.get("params")
        if not isinstance(parameters, list) or len(parameters) != int(
            recorded_group["parameter_count"]
        ):
            raise ArtifactCompatibilityError(
                "normalized-head optimizer state parameter partition changed"
            )
    return dict(value)
...
def _history_diagnostics(
    ...
    diagnostics = {
        ...
        "reconstructed_legacy_preclip_gradient_norm_quantiles": _quantiles(
            [
                float(
                    value.get(
                        "reconstructed_legacy_preclip_gradient_norm", math.nan
                    )
                )
                for value in history
            ]
        ),
        "normalized_body_gradient_norm_quantiles": _quantiles(
            [
                float(value.get("normalized_body_gradient_norm", math.nan))
                for value in history
            ]
        ),
        "normalized_head_gradient_norm_quantiles": _quantiles(
            [
                float(value.get("normalized_head_gradient_norm", math.nan))
                for value in history
            ]
        ),
...
def _normalized_boundary_certificate(model: nn.Module) -> dict[str, Any]:
    finite = all(
        bool(torch.isfinite(value.detach()).all()) for value in model.parameters()
    )
    version = getattr(model, "model_version", None)
    features = tuple(getattr(model, "state_feature_names", ()))
    passed = bool(
        isinstance(model, D0BoundarySmoothMeanHeadPotentialUNet)
        and version == NORMALIZED_HEAD_MODEL_VERSION
        and getattr(model, "head_coordinate_version", None)
        == NORMALIZED_HEAD_COORDINATE_VERSION
        and getattr(model, "scalar_reduction", None) == "spatial_mean"
        and features == ("relative_density", "log1p_relative_density")
        and finite
    )
    return {
        "model_version": version,
        "expected_model_version": NORMALIZED_HEAD_MODEL_VERSION,
        "head_coordinate_version": getattr(model, "head_coordinate_version", None),
        "state_feature_names": list(features),
        "scalar_reduction": getattr(model, "scalar_reduction", None),
        "raw_log_density_used": 0,
        "finite_parameters": int(finite),
        "structural_facet_certificate": (
            "smooth-closed-simplex-inputs-plus-exact-head-equivalence-v1"
        ),
        "passed": int(passed),
    }
...
def run_paired_density_ratio_task(
    *,
    ...
    for step in range(completed + 1, total_steps + 1):
        ...
        reconstructed_legacy_raw = (
            reconstructed_legacy_preclip / float(PARENT_LOSS_SCALE)
        )
        ...
        history.append(
            {
                ...
                **{
                ...
                "reconstructed_legacy_raw_gradient_norm": (
                    reconstructed_legacy_raw
                ),
    ...
    boundary = _normalized_boundary_certificate(model)
    ...
    for candidate_index in (2, 3):
        for task in ("bounded_teacher", "dirichlet_null"):
            model, source = _load_parent_forensic_model(
                args.parent_paired_ratio_run_dir,
                task=task,
...
def _relative_tensor_error(left: Tensor, right: Tensor) -> float:
    difference = torch.linalg.vector_norm((left - right).reshape(-1).to(torch.float64))
    scale = torch.linalg.vector_norm(left.reshape(-1).to(torch.float64)).clamp_min(1e-30)
    return float((difference / scale).detach().cpu())


def _mapping_relative_error(
    expected: Mapping[str, Tensor], actual: Mapping[str, Tensor]
) -> float:
    if set(expected) != set(actual):
        return math.inf
    return max(
        _relative_tensor_error(expected[name], actual[name])
        for name in expected
        if isinstance(expected[name], Tensor) and isinstance(actual[name], Tensor)
    )


def _load_parent_step_payload(
    parent_run_dir: Path,
    *,
    ...
    path = (
        parent_run_dir / "pilot" / "accum-08"
        / f"lr-{int(learning_rate_index):02d}" / "bounded_teacher"
        / "checkpoints" / f"step-{int(step):08d}.pt"
    )
    if not path.is_file():
        raise ArtifactCompatibilityError(
            f"parent accumulation-eight checkpoint is missing: {path}"
        )
    try:
    ...
    if not isinstance(payload, Mapping) or "model_state_dict" not in payload:
        raise ArtifactCompatibilityError("parent checkpoint payload is invalid")
    return path, dict(payload)


def _backward_on_paired_batches(
    model: nn.Module,
    batches: Sequence[PairedMixtureMicrobatch],
) -> tuple[float, dict[str, Tensor]]:
    model.zero_grad(set_to_none=True)
    losses: list[float] = []
    for batch in batches:
        objective, _ = _microbatch_objective_and_components(
            model, batch, task="bounded_teacher"
        )
        losses.append(float(objective.detach().cpu()))
        (objective * float(PARENT_LOSS_SCALE) / float(len(batches))).backward()
    gradients = {
...
def _run_head_coordinate_forensics(
    run_dir: Path,
    *,
    args: argparse.Namespace,
    dynamics: DirectFluxMNISTConfig,
    ...
    """Prove the sum/mean coordinate and diagnose the immutable parent."""

    checkpoint_steps = (250, 500, 1_000, 1_500, 2_000)
    summary_rows: list[dict[str, Any]] = []
    layer_rows: list[dict[str, Any]] = []
    head_scale_errors: list[float] = []
    body_errors: list[float] = []
    finite = True
    first_payload: dict[str, Any] | None = None
    first_batches: list[PairedMixtureMicrobatch] | None = None

    for rate_index, learning_rate in enumerate((3e-5, 1e-5)):
        for step in checkpoint_steps:
            checkpoint_path, payload = _load_parent_step_payload(
                args.parent_paired_ratio_run_dir,
                learning_rate_index=rate_index,
                step=step,
                device=device,
            )
            ...
            batches = [
                generate_paired_mixture_microbatch(
                    paired_stream_plan,
                    phase="normalized-head-parent-forensic",
            ...
            legacy = D0BoundarySmoothPotentialUNet(
                dynamics, base_channels=int(args.base_channels)
            ).to(device)
            legacy.load_state_dict(payload["model_state_dict"], strict=True)
            normalized = D0BoundarySmoothMeanHeadPotentialUNet(
                dynamics, base_channels=int(args.base_channels)
            ).to(device)
            normalized.load_state_dict(
                legacy_state_dict_to_normalized_head(
                    payload["model_state_dict"], int(args.grid_size)
                ),
                strict=True,
            )
            legacy_loss, legacy_gradients = _backward_on_paired_batches(
                legacy, batches
            )
            normalized_loss, normalized_gradients = _backward_on_paired_batches(
                normalized, batches
            )
            normalized_diagnostics = normalized_gradient_diagnostics(normalized)
            legacy_sq = sum(
                float(value.to(torch.float64).square().sum().detach().cpu())
                for value in legacy_gradients.values()
            )
            legacy_head_sq = sum(
                float(
                ...
                for name in ("out.weight", "out.bias")
            )
            legacy_norm = math.sqrt(legacy_sq)
            head_share = legacy_head_sq / legacy_sq if legacy_sq > 0.0 else 0.0
            factor = float(int(args.grid_size) ** 2)
            checkpoint_head_errors: list[float] = []
            checkpoint_body_errors: list[float] = []
            head_difference_sq = 0.0
            head_expected_sq = 0.0
            body_difference_sq = 0.0
            body_expected_sq = 0.0
            for name, legacy_gradient in legacy_gradients.items():
                normalized_gradient = normalized_gradients[name]
                expected = (
                    legacy_gradient / factor
                    if name in {"out.weight", "out.bias"}
                    else legacy_gradient
                )
                error = _relative_tensor_error(expected, normalized_gradient)
                if name in {"out.weight", "out.bias"}:
                    checkpoint_head_errors.append(error)
                    head_difference_sq += float(
                        (expected - normalized_gradient)
                        .to(torch.float64)
                        .square()
                        .sum()
                        .detach()
                    ...
                    head_expected_sq += float(
                        expected.to(torch.float64).square().sum().detach().cpu()
                    )
                else:
                    checkpoint_body_errors.append(error)
                    body_difference_sq += float(
                        (expected - normalized_gradient)
                        .to(torch.float64)
                        .square()
                        .sum()
                ...
                layer_rows.append(
                    {
                        ...
                        "parameter": name,
                        "parameter_count": int(legacy_gradient.numel()),
                        "legacy_gradient_norm": float(
                            torch.linalg.vector_norm(
                                legacy_gradient.reshape(-1).to(torch.float64)
                            ).detach().cpu()
                        ),
                        "normalized_gradient_norm": float(
                            torch.linalg.vector_norm(
                                normalized_gradient.reshape(-1).to(torch.float64)
                            ).detach().cpu()
                        ),
                        "coordinate_relative_error": error,
                        "is_head": int(name in {"out.weight", "out.bias"}),
                        "physical_training_performed": 0,
                        "sampling_performed": 0,
                    }
                )
            aggregate_head_error = math.sqrt(head_difference_sq) / max(
                math.sqrt(head_expected_sq), 1e-30
            )
            aggregate_body_error = math.sqrt(body_difference_sq) / max(
                math.sqrt(body_expected_sq), 1e-30
            )
            head_scale_errors.append(aggregate_head_error)
            body_errors.append(aggregate_body_error)
            row = {
                "learning_rate": learning_rate,
                "checkpoint_step": step,
                "checkpoint_sha256": file_fingerprint(checkpoint_path),
                ...
                "bce_max_abs_error": abs(legacy_loss - normalized_loss),
                "legacy_gradient_norm": legacy_norm,
                "normalized_gradient_norm": normalized_diagnostics[
                    "normalized_gradient_norm"
                ],
                "reconstructed_legacy_gradient_norm": normalized_diagnostics[
                    "reconstructed_legacy_gradient_norm"
                ],
                "legacy_head_squared_gradient_share": head_share,
                "head_gradient_scale_relative_error": aggregate_head_error,
                "backbone_gradient_relative_error": aggregate_body_error,
                "physical_training_performed": 0,
                "sampling_performed": 0,
            }
            finite &= all(
                ...
                if key not in {
                    "checkpoint_sha256", "physical_training_performed",
                    "sampling_performed",
                }
            )
    ...
    if first_payload is None or first_batches is None:
        raise ArtifactCompatibilityError("parent checkpoint forensic plan is empty")

    # Function and derivative equivalence on the first immutable parent model.
    def equivalence(dtype: torch.dtype, target_device: torch.device) -> dict[str, float]:
        legacy = D0BoundarySmoothPotentialUNet(
            dynamics, base_channels=int(args.base_channels)
        ).to(device=target_device, dtype=dtype)
        legacy_state = {
            name: value.to(device=target_device, dtype=dtype)
            if isinstance(value, Tensor) and value.is_floating_point()
            else value.to(device=target_device)
            if isinstance(value, Tensor)
            else value
            for name, value in dict(first_payload["model_state_dict"]).items()
        }
        ...
        normalized.load_state_dict(
            legacy_state_dict_to_normalized_head(legacy_state, int(args.grid_size)),
            strict=True,
        )
        source = first_batches[0]
        states = source.reference_states[:4].to(device=target_device, dtype=dtype)
        tau = source.tau[:4].to(device=target_device, dtype=dtype)
        labels = source.labels[:4].to(device=target_device)
        left_states = states.detach().clone().requires_grad_(True)
        right_states = states.detach().clone().requires_grad_(True)
        left_logits = legacy(tau, left_states, labels)
        right_logits = normalized(tau, right_states, labels)
        left_gradient = torch.autograd.grad(left_logits.sum(), left_states)[0]
        right_gradient = torch.autograd.grad(right_logits.sum(), right_states)[0]
        left_edge = edge_difference_channels(left_gradient, int(args.grid_size))
        right_edge = edge_difference_channels(right_gradient, int(args.grid_size))
        left_flux = physical_flux_from_edge_score(
            left_edge, left_states.detach(), dynamics, time_change=1.0
        )
        right_flux = physical_flux_from_edge_score(
            right_edge, right_states.detach(), dynamics, time_change=1.0
        )
        targets = torch.tensor([0.0, 1.0, 0.0, 1.0], device=target_device, dtype=dtype)
        left_bce = torch.nn.functional.binary_cross_entropy_with_logits(
            left_logits, targets
        )
        right_bce = torch.nn.functional.binary_cross_entropy_with_logits(
            right_logits, targets
        )
        coordinate_batch = generate_paired_mixture_microbatch(
            paired_stream_plan,
            phase="normalized-head-gradient-equivalence",
            task="bounded_teacher",
            optimizer_step=1,
            microbatch_index=0,
        ...
        _, legacy_gradients = _backward_on_paired_batches(
            legacy, [coordinate_batch]
        )
        _, normalized_gradients = _backward_on_paired_batches(
            normalized, [coordinate_batch]
        )
        factor = float(int(args.grid_size) ** 2)
        head_difference_sq = head_expected_sq = 0.0
        body_difference_sq = body_expected_sq = 0.0
        for name, legacy_gradient in legacy_gradients.items():
            expected = (
                legacy_gradient / factor
                if name in {"out.weight", "out.bias"}
            ...
            difference_sq = float(
                (expected - normalized_gradients[name])
                .to(torch.float64)
                .square()
                .sum()
                .detach()
            ...
            if name in {"out.weight", "out.bias"}:
                head_difference_sq += difference_sq
                head_expected_sq += expected_sq
            else:
                body_difference_sq += difference_sq
                body_expected_sq += expected_sq
        result = {
            "logit_max_abs_error": float(
                (left_logits - right_logits).abs().max().detach().cpu()
            ),
            "bce_max_abs_error": float((left_bce - right_bce).abs().detach().cpu()),
            "state_gradient_relative_error": _relative_tensor_error(
                left_gradient, right_gradient
            ),
            "edge_score_relative_error": _relative_tensor_error(
                left_edge, right_edge
            ),
            "flux_relative_error": _relative_tensor_error(left_flux, right_flux),
            "head_gradient_scale_relative_error": math.sqrt(
                head_difference_sq
            ) / max(math.sqrt(head_expected_sq), 1e-30),
            "backbone_gradient_relative_error": math.sqrt(
                body_difference_sq
            ) / max(math.sqrt(body_expected_sq), 1e-30),
        }
        del legacy, normalized
        return result

    float64_equivalence = equivalence(torch.float64, torch.device("cpu"))
    device_equivalence = equivalence(torch.float32, device)

    # Multi-step optimizer/EMA conjugacy in float64, before global clipping.
    legacy = D0BoundarySmoothPotentialUNet(
        dynamics, base_channels=int(args.base_channels)
    ...
    legacy.load_state_dict(legacy_state64, strict=True)
    normalized = D0BoundarySmoothMeanHeadPotentialUNet(
        dynamics, base_channels=int(args.base_channels)
    ).to(dtype=torch.float64)
    normalized.load_state_dict(
        legacy_state_dict_to_normalized_head(legacy_state64, int(args.grid_size)),
        strict=True,
    )
    legacy_optimizer = torch.optim.AdamW(
        legacy.parameters(), lr=3e-5, eps=1e-8, weight_decay=1e-4
    )
    normalized_optimizer = build_coordinate_conjugate_adamw(
        normalized, body_lr=3e-5, eps=1e-8, weight_decay=1e-4
    )
    legacy_ema = init_ema_state(legacy)
    normalized_ema = init_ema_state(normalized)
    adam_errors: list[float] = []
    ema_errors: list[float] = []
    for optimizer_step in (1, 2, 3, 4):
        batch = generate_paired_mixture_microbatch(
            paired_stream_plan,
            phase="normalized-head-adamw-conjugacy",
            task="bounded_teacher",
        ...
        left = _microbatch_objective(legacy, batch, task="bounded_teacher")
        right = _microbatch_objective(normalized, batch, task="bounded_teacher")
        (left * float(PARENT_LOSS_SCALE)).backward()
        (right * float(PARENT_LOSS_SCALE)).backward()
        legacy_optimizer.step()
        normalized_optimizer.step()
        update_ema_state(legacy_ema, legacy, float(args.ema_decay))
        update_ema_state(normalized_ema, normalized, float(args.ema_decay))
        normalized_as_legacy = normalized_state_dict_to_legacy_head(
            normalized.state_dict(), int(args.grid_size)
        )
        normalized_ema_as_legacy = normalized_ema_state_to_legacy_head(
            normalized_ema, int(args.grid_size)
        )
        adam_errors.append(
            _mapping_relative_error(legacy.state_dict(), normalized_as_legacy)
        )
        ema_errors.append(_mapping_relative_error(legacy_ema, normalized_ema_as_legacy))
    optimizer_record = coordinate_conjugate_adamw_record(
        normalized, body_lr=3e-5, eps=1e-8, weight_decay=1e-4
    )
    group_records = {value["name"]: value for value in optimizer_record["groups"]}
    body_group = group_records["body"]
    head_group = group_records["normalized_head"]
    factor = float(int(args.grid_size) ** 2)
    optimizer_group_checks = {
        "adamw_group_learning_rate_pass": int(
            math.isclose(float(head_group["lr"]), factor * float(body_group["lr"]), rel_tol=0.0, abs_tol=0.0)
        ),
        "adamw_group_epsilon_pass": int(
            math.isclose(float(head_group["eps"]), float(body_group["eps"]) / factor, rel_tol=0.0, abs_tol=0.0)
        ),
        "adamw_group_weight_decay_pass": int(
            math.isclose(float(head_group["weight_decay"]), float(body_group["weight_decay"]) / factor, rel_tol=0.0, abs_tol=0.0)
        ),
    }

    head_shares = [float(value["legacy_head_squared_gradient_share"]) for value in summary_rows]
    width_counts = {
        str(width): sum(
            parameter.numel()
            for parameter in D0BoundarySmoothMeanHeadPotentialUNet(
                dynamics, base_channels=width
            ).parameters()
        )
        for width in (16, 24, 32)
    }
    equivalence_record = {
        "schema": RUN_SCHEMA + "-head-equivalence",
        "schema_version": 1,
        "float64": float64_equivalence,
        "device": device_equivalence,
        "device_type": device.type,
        "head_gradient_scale_relative_error": float64_equivalence[
            "head_gradient_scale_relative_error"
        ],
        "backbone_gradient_relative_error": float64_equivalence[
            "backbone_gradient_relative_error"
        ],
        "parent_cuda_head_gradient_scale_relative_error_advisory": max(
            head_scale_errors, default=math.inf
        ),
        "parent_cuda_backbone_gradient_relative_error_advisory": max(
            body_errors, default=math.inf
        ),
    ...
    conjugacy_record = {
        "schema": RUN_SCHEMA + "-optimizer-conjugacy",
        "schema_version": 1,
        "steps": 4,
        "adamw_coordinate_max_relative_error": max(adam_errors, default=math.inf),
        "ema_coordinate_max_relative_error": max(ema_errors, default=math.inf),
    ...
    forensic_record = {
        "schema": RUN_SCHEMA + "-parent-head-gradient-forensics",
        "schema_version": 1,
        "finite": int(finite),
        "checkpoint_count": len(summary_rows),
        "median_legacy_head_squared_gradient_share": float(np.median(head_shares)),
        "minimum_legacy_head_squared_gradient_share": min(head_shares),
        "maximum_legacy_head_squared_gradient_share": max(head_shares),
        "width_parameter_counts_advisory": width_counts,
        "width_ablation_performed": 0,
        "physical_training_performed": 0,
        "sampling_performed": 0,
    }
    atomic_write_csv(run_dir / "parent_head_gradient_forensics.csv", summary_rows)
    atomic_write_csv(run_dir / "parent_head_layerwise_gradients.csv", layer_rows)
    atomic_write_json(run_dir / "head_equivalence.json", equivalence_record)
    atomic_write_json(run_dir / "adamw_ema_conjugacy.json", conjugacy_record)
    atomic_write_json(run_dir / "model_size_forensics.json", forensic_record)
    return {
        "float64_logit_max_abs_error": float64_equivalence["logit_max_abs_error"],
        "float64_bce_max_abs_error": float64_equivalence["bce_max_abs_error"],
        "cuda_logit_max_abs_error": device_equivalence["logit_max_abs_error"],
        "cuda_bce_max_abs_error": device_equivalence["bce_max_abs_error"],
        "state_gradient_relative_error": device_equivalence[
            "state_gradient_relative_error"
        ],
        "edge_score_relative_error": device_equivalence["edge_score_relative_error"],
        "flux_relative_error": device_equivalence["flux_relative_error"],
        "head_gradient_scale_relative_error": equivalence_record[
            "head_gradient_scale_relative_error"
        ],
        "backbone_gradient_relative_error": equivalence_record[
            "backbone_gradient_relative_error"
        ],
        "adamw_coordinate_max_relative_error": conjugacy_record[
            "adamw_coordinate_max_relative_error"
        ],
        "ema_coordinate_max_relative_error": conjugacy_record[
            "ema_coordinate_max_relative_error"
        ],
        **optimizer_group_checks,
        "parent_forensic_finite": int(finite),
        "median_legacy_head_squared_gradient_share": forensic_record[
            "median_legacy_head_squared_gradient_share"
        ],
        "legacy_checkpoint_report_only_pass": int(
            first_payload.get("model_schema") != NORMALIZED_HEAD_MODEL_VERSION
        ),
    }
...
def _run_preflight(
    ...
    *,
    ...
    coordinate = _run_head_coordinate_forensics(
        run_dir,
        args=args,
        dynamics=dynamics,
        device=device,
    ...
    boundary = _normalized_boundary_certificate(
        D0BoundarySmoothMeanHeadPotentialUNet(
            dynamics, base_channels=int(args.base_channels)
        )
    )
    base._set_seed(base._derived_seed(int(args.root_seed), "preflight", "device"))
    smoke_model = D0BoundarySmoothMeanHeadPotentialUNet(
        dynamics, base_channels=int(args.base_channels)
    ).to(device)
    ...
    metrics = {
        ...
        "grid_cells": int(args.grid_size) ** 2,
        "base_channels": int(args.base_channels),
        ...
        "fresh_panel_isolation_pass": int(seed_namespace_pass),
        "boundary_operator_pass": int(bool(boundary["passed"]) and parent_boundary),
        "device_smoke_pass": int(device_pass),
        "finite_device_backward_pass": int(device_pass),
        "paired_estimator_pass": int(
            loss_algebra_error <= 1e-12
            and expanded_loss_error <= 1e-7
            and expanded_gradient_error <= 1e-6
            and accumulation_error <= 1e-6
            and common_gamma_pass
            and replay_pass
        ),
        "head_gradient_one_over_n_pass": int(
            float(coordinate["head_gradient_scale_relative_error"])
            <= float(thresholds.gradient_scaling_tolerance)
        ),
        "backbone_gradient_unchanged_pass": int(
            float(coordinate["backbone_gradient_relative_error"])
            <= float(thresholds.gradient_scaling_tolerance)
        ),
        **coordinate,
    ...
    gate = evaluate_normalized_head_preflight(metrics, thresholds)
    atomic_write_json(run_dir / "normalized_head_preflight.json", metrics)
    atomic_write_json(run_dir / "normalized_head_preflight_gate.json", gate)
    return gate
...
def _freeze_selected_profile(
    ...
    path = run_dir / "selected_normalized_head_profile.json"
    if path.is_file():
...
def _bind_confirmation_profile(
    ...
    profile_path = run_dir / "selected_normalized_head_profile.json"
    pilot_path = run_dir / "normalized_head_pilot_gate.json"
    normalized = json.loads(json.dumps(dict(selected), sort_keys=True, allow_nan=False))
...
def _run_pilot(
    ...
    *,
    ...
    for level_index, accumulation_level in enumerate(args.accumulation_levels):
        # The coordinate-repair gate freezes accumulation at eight.  Both
        # learning-rate candidates share one stateless training stream.
        ...
        for rate_index, learning_rate in enumerate(args.pilot_learning_rates):
            ...
            candidate_gate = evaluate_head_pilot_candidate(candidate, thresholds)
            candidate["gate"] = candidate_gate
            level_candidates.append(candidate)
            candidates.append(candidate)
        atomic_write_json(
            run_dir / f"normalized_head_pilot_accum_{int(accumulation_level)}.json",
            {
    ...
    pilot = evaluate_head_pilot(candidates, thresholds)
    pilot["candidate_records"] = candidates
    atomic_write_json(run_dir / "normalized_head_pilot_gate.json", pilot)
    atomic_write_json(
...
def _run_confirmation(
    ...
    *,
    ...
    atomic_write_json(
        run_dir / "normalized_head_teacher_confirmation.json",
        {
    ...
    atomic_write_json(
        run_dir / "normalized_head_null_confirmation.json",
        {
...
def _load_report_inputs(
    ...
    preflight_path = run_dir / "normalized_head_preflight_gate.json"
    pilot_path = run_dir / "normalized_head_pilot_gate.json"
    teacher_path = run_dir / "normalized_head_teacher_confirmation.json"
    null_path = run_dir / "normalized_head_null_confirmation.json"
    preflight = (
        _json_load(preflight_path)
        if preflight_path.is_file()
        else _not_evaluated_gate("normalized_head_preflight", "preflight was not run")
    )
    pilot = (
        _json_load(pilot_path)
        if pilot_path.is_file()
        else _not_evaluated_gate("normalized_head_pilot", "pilot was not run")
    )
    teacher = (
...
def _mark_interim_stage_success(
    ...
    if stage == "preflight":
        ...
        action = "run the normalized-head optimizer pilot"
    elif stage == "pilot":
...
def _save_report(run_dir: Path, report: Mapping[str, Any]) -> None:
    atomic_write_json(run_dir / "normalized_head_control_gate.json", report)
    atomic_write_json(run_dir / "normalized_head_decision.json", report["decision"])
...
def _write_summary_csvs(run_dir: Path) -> list[str]:
    ...
    for filename, task in (
        ("normalized_head_teacher_confirmation.json", "bounded_teacher"),
        ("normalized_head_null_confirmation.json", "dirichlet_null"),
    ):
...
def _read_history_columns(
    path: Path, names: Sequence[str]
) -> dict[str, np.ndarray]:
    columns: dict[str, list[float]] = {str(name): [] for name in names}
    with path.open("r", encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle)
        if reader.fieldnames is None or any(name not in reader.fieldnames for name in names):
            raise ValueError(f"training history lacks required columns: {tuple(names)}")
        for row in reader:
            for name in names:
...
def _write_plot_artifacts(run_dir: Path) -> list[str]:
    try:
        ...
        return []
    written: list[str] = []
    forensic_path = run_dir / "parent_head_gradient_forensics.csv"
    if forensic_path.is_file():
        try:
            data = np.atleast_1d(
                np.genfromtxt(forensic_path, delimiter=",", names=True)
            )
            figure, (norm_axis, share_axis) = plt.subplots(
                2, 1, figsize=(8, 7), sharex=True
            )
            for learning_rate in sorted(set(data["learning_rate"].tolist())):
                selected = data[data["learning_rate"] == learning_rate]
                order = np.argsort(selected["checkpoint_step"])
                selected = selected[order]
                label = f"body lr={learning_rate:g}"
                norm_axis.plot(
                    selected["checkpoint_step"],
                    selected["legacy_gradient_norm"],
                    marker="o",
                    label=f"legacy, {label}",
                )
                norm_axis.plot(
                    selected["checkpoint_step"],
                    selected["normalized_gradient_norm"],
                    marker="o",
                    linestyle="--",
                ...
                share_axis.plot(
                    selected["checkpoint_step"],
                    selected["legacy_head_squared_gradient_share"],
                    marker="o",
                    label=label,
                )
            norm_axis.set_yscale("log")
            norm_axis.set_ylabel("scaled gradient norm")
            norm_axis.set_title("Exact parent head-coordinate forensic replay")
            norm_axis.legend(fontsize=7)
            share_axis.axhline(0.95, color="black", linestyle=":", linewidth=1)
            share_axis.set_ylabel("legacy head squared share")
            share_axis.set_xlabel("parent checkpoint step")
            share_axis.set_ylim(0.94, 1.001)
            share_axis.legend(fontsize=7)
            figure.tight_layout()
            output = run_dir / "normalized_head_gradient_forensics.png"
    ...
    if histories:
        ...
        for path in histories:
            try:
                data = _read_history_columns(path, ("step", "scaled_loss"))
                axis.plot(
                    data["step"],
                    data["scaled_loss"],
                    alpha=0.65,
        ...
        axis.set_title("Normalized-head paired-mixture pilot")
        if 0 < plotted <= 12:
            axis.legend(fontsize=6)
        figure.tight_layout()
        output = run_dir / "normalized_head_pilot_learning_curves.png"
        base._atomic_save_figure(figure, output)
        plt.close(figure)
        written.append(output.name)
    ...
    if confirmation_histories:
        ...
        for path in confirmation_histories:
            try:
                ...
                gradients = data["scaled_preclip_gradient_norm"]
        ...
        loss_axis.set_title("Normalized-head confirmation learning curves")
        ...
        figure.tight_layout()
        output = run_dir / "normalized_head_confirmation_optimizer_health.png"
        base._atomic_save_figure(figure, output)
        plt.close(figure)
        written.append(output.name)
    return written
...
def _run(args: argparse.Namespace) -> int:
    ...
    if not args.no_progress:
        print(f"normalized-head density-ratio run directory: {run_dir.resolve()}", flush=True)
    thresholds = HeadCoordinateThresholds()
    ...
    try:
        ...
        parent = verify_parent_paired_ratio_run(args.parent_paired_ratio_run_dir)
        provenance = {
            **dict(parent),
        ...
        plans_path = run_dir / "normalized_head_stream_plans.json"
        ...
        pilot = _not_evaluated_gate("normalized_head_pilot", "pilot was not run")
        teacher_results: list[dict[str, Any]] = []
        null_results: list[dict[str, Any]] = []
                ...
                skips=([] if bool(int(preflight.get("passed", 0))) else [
                    {
                        ...
                        "reason": "normalized-head preflight failed",
                    }
                ]),
            )
            ...
            if args.stage == "pilot" or not bool(int(pilot.get("passed", 0))):
                ...
                return _finish(
                    ...
                    skips=([] if bool(int(pilot.get("passed", 0))) else [
                        {
                            "stage": "confirmation",
                            "reason": "no normalized-head profile qualified",
                        }
                    ]),
                )
        else:
        ...
        if resumed and not mutation_started:
            if not args.no_progress:
                print(
                    f"normalized-head resume rejected: {type(exc).__name__}: {exc}",
                    file=sys.stderr,
                )
            return 2
        atomic_write_json(
        ...
        if not args.no_progress:
            print(
                f"normalized-head control failed: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
        return 2

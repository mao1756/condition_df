"""Restartable EMA-proximal H1 tasks for D0 density-ratio controls.

This module is deliberately additive.  It reuses the normalized-head paired
BCE task semantics while introducing a new checkpoint schema for the moving
``raw - stopgrad(EMA)`` H1 function-step trust term.  It never imports or
invokes a reverse sampler.
"""
...
H1_TASK_SCHEMA = "experiment12-d0-score-density-ratio-h1-task"
H1_TASK_SCHEMA_VERSION = 1
H1_TASK_CHECKPOINT_SCHEMA_VERSION = 1
H1_TASK_TRAINING_VERSION = "d0-paired-bce-stopped-ema-h1-training-v1"
...
__all__ = [
    ...
    "h1_task_fingerprints",
    "write_failed_h1_task_result",
    "run_h1_paired_density_ratio_task",
]
...
def h1_task_fingerprints(
    ...
    *,
    ...
    """Bind one task to the immutable H1 operator, stream and calibration."""
    ...
    additions = {
        ...
        "h1_effective_multiplier": ratio
        * float(calibration_record["lambda_base"]),
...
def write_failed_h1_task_result(
    task_dir: Path,
    *,
    task: str,
    model_seed: int,
...
def run_h1_paired_density_ratio_task(
    *,
    task_dir: Path,
    task: str,
    ...
    if not math.isfinite(ratio) or ratio < 0.0:
        raise ValueError("h1_ratio must be finite and nonnegative")
    calibration_record = _calibration_record(calibration)
    bound_fingerprints = h1_task_fingerprints(
        fingerprints,
        trust_plan=trust_plan,
    ...
    effective_multiplier = float(calibration_record["lambda_base"]) * ratio
    ...
    for step in range(completed + 1, total_steps + 1):
            ...
            if ratio > 0.0:
                ...
                (
                    ...
                    * components.objective
                    / float(trust_plan.banks_per_update)
                ).backward()
        ...
        if ratio > 0.0:
            ...
            trust_loss = effective_multiplier * trust_objective
            if not math.isfinite(float(trust_loss.detach().cpu())):
                raise FloatingPointError(f"nonfinite H1 loss for {task} at step {step}")
        ...
        h1_scaled_norm = float(geometry["right_norm"])
        ...
        history.append(
            {
                ...
                **{
                ...
                "h1_multiplier": ratio,
                "h1_lambda_base": float(calibration_record["lambda_base"]),
                "h1_effective_multiplier": effective_multiplier,
    ...
    if task == "bounded_teacher" and isinstance(
        panel_b_record.get("analytic"), Mapping
    ):
    ...
    gate = (
        ...
        if task == "dirichlet_null" and audit_panels is not None
            ...
            "gate": "h1_density_ratio_pilot_task",
            "evaluation_status": "evaluated",
